...
class Database:
    ...
    def register_user(self, user_id, username, referrer_id=None):
        ...
        if not self.cursor.fetchone():
            ...
            if referrer_id:
                date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                self.cursor.execute(
                    "INSERT INTO referrals (user_id, referrer_id, level, date) VALUES (?, ?, ?, ?)",
                    (user_id, referrer_id, 1, date)
                )
                
                # Добавляем рефералов 2 и 3 уровня
                self.cursor.execute("SELECT referrer_id FROM referrals WHERE user_id = ?", (referrer_id,))
                level2_ref = self.cursor.fetchone()
                if level2_ref and level2_ref[0]:
                    self.cursor.execute(
                        "INSERT INTO referrals (user_id, referrer_id, level, date) VALUES (?, ?, ?, ?)",
                        (user_id, level2_ref[0], 2, date)
                    )
                    
                    self.cursor.execute("SELECT referrer_id FROM referrals WHERE user_id = ?", (level2_ref[0],))
                    level3_ref = self.cursor.fetchone()
                    if level3_ref and level3_ref[0]:
                        self.cursor.execute(
                            "INSERT INTO referrals (user_id, referrer_id, level, date) VALUES (?, ?, ?, ?)",
                            (user_id, level3_ref[0], 3, date)
                        )
                self.conn.commit()
            return True
        return False
    ...
    def get_ref_stats(self, user_id):
        # Получаем количество рефералов 1 уровня
        self.cursor.execute("SELECT COUNT(*) FROM referrals WHERE referrer_id = ? AND level = 1", (user_id,))
        level1 = self.cursor.fetchone()[0]
        
        # Получаем количество рефералов 2 уровня
        self.cursor.execute("SELECT COUNT(*) FROM referrals WHERE referrer_id = ? AND level = 2", (user_id,))
        level2 = self.cursor.fetchone()[0]
        
        # Получаем количество рефералов 3 уровня
        self.cursor.execute("SELECT COUNT(*) FROM referrals WHERE referrer_id = ? AND level = 3", (user_id,))
        level3 = self.cursor.fetchone()[0]
...
# Функция для форматирования текста с премиум-эмодзи
def format_text_with_emojis(text, user_id=None):
    """Заменяет маркеры вида {emoji_name} на HTML теги премиум-эмодзи"""
    if not text:
        return text
    
    # Получаем баланс эмодзи для подстановки
    balance_emoji = get_balance_emoji(user_id) if user_id else "{balance_emoji}"
    
    for emoji_name, emoji_data in config.PREMIUM_EMOJIS.items():
        emoji_id = emoji_data[:-1]  # ID эмодзи (все кроме последнего символа)
        emoji_char = emoji_data[-1]  # Последний символ - сам эмодзи
        placeholder = "{" + emoji_name + "}"
        html_tag = f'<tg-emoji emoji-id="{emoji_id}">{emoji_char}</tg-emoji>'
        text = text.replace(placeholder, html_tag)
    
    # Заменяем специальные плейсхолдеры
    text = text.replace("{balance_emoji}", balance_emoji)
    text = text.replace("{channel_url}", config.CHANNEL_URL)
    
    return text

def get_balance_emoji(user_id=None):
    """Возвращает HTML тег для эмодзи баланса"""
    emoji_data = config.PREMIUM_EMOJIS.get("balance", "5262509177363787445💰")
    emoji_id = emoji_data[:-1]
    emoji_char = emoji_data[-1]
...
def get_btn(user_id: int, key: str, **kwargs) -> str:
    """Возвращает текст кнопки по ключу с заменой эмодзи"""
    lang = get_lang(user_id)
    ...
    if kwargs:
        try:
            ...
            logger.error(f"Missing key in button formatting: {e}")
        except Exception as e:
...
def get_main_keyboard(user_id: int):
    ...
    if user_id in config.ADMINS:
        builder.row(
            InlineKeyboardButton(text="⚙️ Admin", callback_data=f"admin_menu:{user_id}")
        )
    ...
    if is_new and referrer_id:
        try:
            ...
            await message.bot.send_message(referrer_id, 
                f"👤 У вас новый реферал: <b>{username}</b>!")
        except:
            pass
...
async def admin_command_handler(message: Message, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="📢 Рассылка", callback_data=f"admin_broadcast:{user_id}"),
        InlineKeyboardButton(text="💰 Прибавить баланс", callback_data=f"admin_add_balance:{user_id}")
    )
    builder.row(
        InlineKeyboardButton(text="⚖️ Установить баланс", callback_data=f"admin_set_balance:{user_id}"),
        InlineKeyboardButton(text="📊 Статистика", callback_data=f"admin_stats:{user_id}")
    )
    builder.row(
        InlineKeyboardButton(text=get_btn(user_id, "back"), callback_data=f"main_menu:{user_id}")
    )
    
    await message.answer(
        get_text(user_id, "admin_menu"),
...
async def admin_broadcast_callback(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="❌ Отмена", callback_data=f"admin_menu:{owner_id}"))
    
    await callback.message.edit_text(
        get_text(owner_id, "admin_broadcast"),
        reply_markup=builder.as_markup(),
        parse_mode=ParseMode.HTML
    )
...
async def admin_broadcast_process(message: Message, state: FSMContext):
    ...
    if message.text == "/cancel":
        await state.clear()
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="⬅️ Назад", callback_data=f"admin_menu:{user_id}"))
        return await message.answer(
            get_text(user_id, "admin_broadcast_cancel"),
            reply_markup=builder.as_markup(),
            parse_mode=ParseMode.HTML
        )
    ...
    for uid in users:
        try:
            await message.bot.send_message(uid, message.text, parse_mode=ParseMode.HTML)
            sent_count += 1
    ...
    await message.answer(
        get_text(user_id, "admin_broadcast_sent").format(count=sent_count),
        reply_markup=builder.as_markup(),
        parse_mode=ParseMode.HTML
    )

@dp.callback_query(F.data.startswith("admin_add_balance:"))
async def admin_add_balance_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        get_text(owner_id, "admin_add_balance"),
        reply_markup=builder.as_markup(),
        parse_mode=ParseMode.HTML
    )

@dp.message(AdminState.add_balance)
async def admin_add_balance_process(message: Message, state: FSMContext):
    ...
    try:
        ...
        if len(parts) != 2:
            return await message.answer(get_text(user_id, "admin_invalid_format"))
        ...
        if not user_data:
            return await message.answer(
                get_text(user_id, "admin_user_not_found").format(user_id=target_id),
                parse_mode=ParseMode.HTML
            )
        
        # Добавляем баланс
        db.add_balance(target_id, amount, is_deposit=True)
        ...
        await message.answer(
            get_text(user_id, "admin_balance_added").format(user_id=target_id, amount=amount),
            reply_markup=builder.as_markup(),
            parse_mode=ParseMode.HTML
        )
        ...
        try:
            await message.bot.send_message(target_id,
                f"{get_balance_emoji()} Ваш баланс был пополнен администратором на <b>{amount:.2f} {get_balance_emoji()}</b>!",
                parse_mode=ParseMode.HTML
            )
        except:
            pass
        ...
        await message.answer(get_text(user_id, "admin_invalid_format"))
    except Exception as e:
        await message.answer(f"❌ Ошибка: {e}")

@dp.callback_query(F.data.startswith("admin_set_balance:"))
async def admin_set_balance_callback(callback: CallbackQuery, state: FSMContext):
    """Начало установки баланса"""
    owner_id = int(callback.data.split(":")[-1])
    if not await check_owner(callback, owner_id):
        return
    ...
    await callback.message.edit_text(
        get_text(owner_id, "admin_set_balance"),
        reply_markup=builder.as_markup(),
        parse_mode=ParseMode.HTML
    )

@dp.message(AdminState.set_balance)
async def admin_set_balance_process(message: Message, state: FSMContext):
    ...
    try:
        ...
        if diff >= 0:
            db.add_balance(target_id, diff)
        else:
            db.add_balance(target_id, diff, is_withdraw=True)
        ...
        await message.answer(
            get_text(user_id, "admin_balance_set").format(user_id=target_id, amount=new_balance),
            reply_markup=builder.as_markup(),
            parse_mode=ParseMode.HTML
        )
        ...
        try:
            await message.bot.send_message(target_id,
                f"{get_balance_emoji()} Ваш баланс был изменен администратором на <b>{new_balance:.2f} {get_balance_emoji()}</b>!",
                parse_mode=ParseMode.HTML
            )
        except:
            pass
...
async def admin_stats_callback(callback: CallbackQuery):
    ...
    for uid in users:
        ...
        if data:
            total_balance += data[3]
    
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="⬅️ Назад", callback_data=f"admin_menu:{owner_id}"))
    
    text = (
        f"<b>📊 Статистика бота</b>\n\n"
        f"👥 Всего пользователей: <b>{total_users}</b>\n"
        f"{get_balance_emoji()} Общий баланс: <b>{total_balance:.2f} {get_balance_emoji()}</b>"
    )
...
# --- Обработчик для установки ставки (исправлен) ---
@dp.message(StateFilter(None), F.text.regexp(r"^(\d+[.,]?\d*)\s*[\$💰]?$"))
async def set_bet_by_text_handler(message: Message, state: FSMContext):
    """Установка ставки через текст (например, '0.1' или '5$')"""
    ...
    # Очищаем текст от валютных символов
    ...
    try:
        ...
        if amount < 0.01:
            return await message.answer(
                f"❌ Минимальная ставка — <b>0.01 {get_balance_emoji()}</b>",
                parse_mode=ParseMode.HTML
            )
        ...
        if amount > config.MAX_BET:
            return await message.answer(
                f"❌ Максимальная ставка — <b>{config.MAX_BET:.2f} {get_balance_emoji()}</b>",
                parse_mode=ParseMode.HTML
            )
        ...
        await message.answer(
            f"✅ Ваша ставка установлена на <b>{amount:.2f} {get_balance_emoji()}</b>",
            parse_mode=ParseMode.HTML
        )
    except ValueError:
        # Если это не число, просто игнорируем
        pass
...
async def dice_text_handler(message: Message, state: FSMContext):
    ...
    if text == "7":
        ...
        return await message.answer(
            ...
            f"<blockquote>Баланс — <b>{balance:.2f}</b> {get_balance_emozi()}\n"
            f"Ставка — <b>{current_bet:.2f}</b> {get_balance_emozi()}</blockquote>\n\n"
...
async def multiply_text_handler(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"<blockquote>Баланс — <b>{balance:.2f}</b> {get_balance_emoji()}\n"
        f"Ставка — <b>{current_bet:.2f}</b> {get_balance_emoji()}</blockquote>\n\n"
        f"<i>Пополняй и сыграй на реальные деньги</i>",
        reply_markup=builder.as_markup(),
        parse_mode=ParseMode.HTML
    )
...
async def text_games_handler(message: Message, user_id: int = None):
    ...
    text = get_text(user_id, "play", balance=balance, bet=current_bet)
...
async def vb_command_handler(message: Message):
    ...
    await message.answer(
        f"Вы действительно хотите поставить весь баланс (<b>{balance:.2f} {get_balance_emoji()}</b>)?",
        reply_markup=builder.as_markup(),
        parse_mode=ParseMode.HTML
    )
...
async def confirm_vb_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
            f"✅ Ваша ставка установлена на: <b>{bet_amount:.2f} {get_balance_emoji()}</b>" + 
...
async def text_balance_handler(message: Message):
    ...
    text = (
        ...
        f"<blockquote><b>{get_balance_emoji()} Баланс — {balance:.2f} {get_balance_emoji()}</b></blockquote>"
    )
...
async def give_balance_handler(message: Message):
    ...
    try:
        ...
        if db.add_balance(target_id, amount):
            await message.answer(f"✅ Баланс игрока <code>{target_id}</code> изменен на <b>{amount:.2f} {get_balance_emoji()}</b>")
...
async def transfer_balance_handler(message: Message):
    ...
    if amount < 0.1:
        return await message.answer(f"❌ Минимальная сумма перевода — <b>0.10 {get_balance_emoji()}</b>", parse_mode=ParseMode.HTML)
    ...
    if db.add_balance(sender_id, -amount, is_withdraw=True):
        ...
        await message.answer(
            f"🎊 {sender_name} передаёт <b>{amount:,.2f} {get_balance_emoji()}</b> {recipient_name}",
            parse_mode=ParseMode.HTML
        )
...
async def stats_callback(callback: CallbackQuery):
    ...
    if lang == "ru":
        ...
        days_label = "days" if days_delta != 1 else "day"
    
    display_name = get_user_display_name(user_id, callback.from_user.first_name)
    stats_text = get_text(user_id, "stats_text",
        name=display_name,
        bets=total_bets,
        turnover=total_turnover,
        days=days_delta,
...
async def deposit_method_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        get_text(user_id, "enter_deposit_amount", min_amount=config.MIN_DEPOSIT),
        reply_markup=builder.as_markup(),
        parse_mode=ParseMode.HTML
    )
...
async def process_deposit_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer(get_text(user_id, "enter_deposit_amount", min_amount=config.MIN_DEPOSIT))

    if amount < config.MIN_DEPOSIT:
        return await message.answer(get_text(user_id, "error_min_deposit", min_amount=config.MIN_DEPOSIT))
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=get_btn(user_id, "pay", amount=amount), url=pay_url))
...
async def check_payment_callback(callback: CallbackQuery):
    ...
    if is_paid:
        ...
        await callback.message.edit_text(
            get_text(user_id, "payment_success", amount=amount),
            parse_mode=ParseMode.HTML
        )
...
async def withdraw_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if balance < config.MIN_WITHDRAW:
        return await callback.answer(get_text(user_id, "error_min_withdraw", min_amount=config.MIN_WITHDRAW), show_alert=True)
    ...
    await callback.message.edit_text(
        get_text(user_id, "enter_withdraw_amount", min_amount=config.MIN_WITHDRAW),
        reply_markup=builder.as_markup(),
        parse_mode=ParseMode.HTML
    )
...
async def process_withdraw_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        if not any(char.isdigit() for char in text):
            return
        return await message.answer(get_text(user_id, "enter_withdraw_amount", min_amount=config.MIN_WITHDRAW))

    if amount < config.MIN_WITHDRAW:
        return await message.answer(get_text(user_id, "error_min_withdraw", min_amount=config.MIN_WITHDRAW))

    if amount > 1000000: # Разумный предел для предотвращения ошибок с гигантскими числами
        return await message.answer("❌ Сумма слишком велика.")

    if amount > balance:
        return await message.answer(f"❌ Недостаточно средств. Ваш баланс: {balance:.2f} {get_balance_emoji()}")
...
async def withdraw_method_callback(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        try:
            ...
            if transfer_success:
                await callback.message.edit_text(
                    f"✅ Вывод <b>{amount:.2f} {get_balance_emoji()}</b> успешно выполнен (прямой перевод)!",
                    parse_mode=ParseMode.HTML
                )
            ...
            if check_url:
                await callback.message.edit_text(
                    f"✅ Чек на сумму <b>{amount:.2f} {get_balance_emoji()}</b> успешно создан!\n\n"
                    f"🔗 Ссылка: {check_url}",
            ...
            admin_text = (
                ...
                f"<blockquote>💵 Сумма: <b>{amount:.2f} {get_balance_emoji()}</b>\n"
...
async def privacy_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        get_text(user_id, "privacy", display_mode=current_display),
        reply_markup=builder.as_markup(),
        parse_mode=ParseMode.HTML
    )
...
async def send_alert(bot: Bot, user_id: int, amount: float, type: str):
    ...
    try:
        ...
        await bot.send_message(chat_id=config.ALERTS_CHANNEL, text=text, parse_mode=ParseMode.HTML)
    except Exception as e:
...
async def referral_callback(callback: CallbackQuery):
    ...
    level1, level2, level3 = db.get_ref_stats(user_id)
    ref_count = level1
    ...
    builder.row(InlineKeyboardButton(
        text=get_btn(user_id, "claim_ref", amount=ref_balance), 
        callback_data=f"claim_ref:{user_id}"
    ))
    builder.row(InlineKeyboardButton(
        text=get_btn(user_id, "invite_friend"), 
        switch_inline_query=f"Играй со мной! {ref_link}"
    ))
    builder.row(InlineKeyboardButton(
        text=get_btn(user_id, "details"), 
        url=config.CHANNEL_URL
    ))
    builder.row(InlineKeyboardButton(
        text=get_btn(user_id, "back"), 
        callback_data=f"main_menu:{user_id}"
    ))

    text = get_text(user_id, "referral",
        ref_count=ref_count,
...
async def claim_ref_callback(callback: CallbackQuery):
    ...
    if claimed > 0:
        await callback.answer(f"✅ Выведено {claimed:.2f} {get_balance_emoji()} на основной баланс!", show_alert=True)
...
async def play_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        get_text(user_id, "play", balance=balance, bet=current_bet), 
        reply_markup=builder.as_markup(), 
        parse_mode=ParseMode.HTML
    )
...
async def modes_menu_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        get_text(user_id, "modes_menu", balance=balance, bet=current_bet), 
        reply_markup=builder.as_markup(), 
        parse_mode=ParseMode.HTML
    )
...
async def show_mines_menu(message: Message, user_id: int, state: FSMContext, edit: bool = True):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=f"🕹️ Играть · {bet:,.2f} {get_balance_emoji()}", callback_data=f"start_mines:{mines_count}:{user_id}"))
    builder.row(
    ...
    text = get_text(user_id, "mines_main",
        player_id=player_id,
        balance=balance,
        bet=bet,
        mines=mines_count
...
async def select_mines_count_handler(callback: CallbackQuery, state: FSMContext):
    ...
    text = get_text(user_id, "mines_select",
        mines=current_mines,
        coefs=coefs_line
    )
...
async def show_mines_field(message: Message, user_id: int, state: FSMContext):
    ...
    # Кнопка Забрать
    builder.row(InlineKeyboardButton(
        text=f"⚡ Забрать · {win_amount:,.2f} {get_balance_emoji()}", 
    ...
    text = get_text(user_id, "mines_playing",
        mines=mines_count,
        bet=bet,
        coef=current_coef,
        win=win_amount,
...
async def mine_click_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if field[idx] == 1: # Попал на мину
            ...
            text = (
                f"👤 <b>{user_name}</b>\n"
                f"<b>Проигрывает в игре 💣 на {game_data['bet']:.2f} {get_balance_emoji()}</b>\n"
                f"<blockquote><b>× 0 🎄 Выигрыш 0.00 {get_balance_emoji()} ❞</b></blockquote>\n\n"
                f"<b>📋 Баланс {new_balance:.2f} {get_balance_emoji()}</b>"
            )
            await callback.message.edit_text(
                text,
                reply_markup=builder.as_markup(),
...
async def mine_cashout_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        text = (
            f"<b>👤 {user_name}</b>\n"
            f"<b>Побеждает в игре 💣 на {bet:.2f} {get_balance_emoji()}</b>\n"
            f"<blockquote><b>× {coef:.2f} 🎄 Выигрыш {win_amount:.2f} {get_balance_emoji()} ❞</b></blockquote>\n\n"
            f"<b>📋 Баланс {new_balance:.2f} {get_balance_emoji()}</b>"
        )
...
async def show_tower_menu(event: CallbackQuery | Message, user_id: int, state: FSMContext, edit=True):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=f"🕹 Играть · {bet:,.2f} {get_balance_emoji()}", callback_data=f"tower_start_game:{bombs_count}:{user_id}"))
    builder.row(
        InlineKeyboardButton(text="⬅️ Назад", callback_data=f"modes_menu:{user_id}"),
...
async def tower_select_bombs_handler(callback: CallbackQuery, state: FSMContext):
    ...
    text = get_text(owner_id, "mines_select",
        mines=current_bombs,
        coefs=coefs_line
    )
...
async def show_tower_field(message: Message, user_id: int, state: FSMContext):
    ...
    text = (
        ...
        f"<b>{bet:,.2f} {get_balance_emoji()} × {current_coef:.2f} ➔ {win_amount:,.2f} {get_balance_emoji()}</b>"
    )
...
async def tower_click_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if field[level][idx] == 1: # Попал на бомбу
            ...
            text = (
                f"👤 <b>{user_name}</b>\n"
                f"<b>Проигрывает в игре 🏙 на {bet:,.2f} {get_balance_emoji()}</b>\n"
                f"<blockquote><b>× 0 🎄 Выигрыш 0.00 {get_balance_emoji()} ❞</b></blockquote>\n\n"
                f"<b>📋 Баланс {new_balance:,.2f} {get_balance_emoji()}</b>"
            )
            await callback.message.edit_text(text, reply_markup=builder.as_markup(), parse_mode=ParseMode.HTML)
...
async def custom_games_menu_handler(callback: CallbackQuery, state: FSMContext):
    ...
    text = get_text(user_id, "play", balance=balance, bet=bet)
...
async def custom_game_play_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        bet_msg_text = (
            f"<b>{user_name} ставит {bet:.2f} {get_balance_emoji()}</b>\n"
        ...
        if is_win:
            ...
            text = (
                f"<b>👤 {user_name}</b>\n"
                f"<b>Побеждает в игре {emoji} на {bet:.2f} {get_balance_emoji()}</b>\n"
                f"<blockquote><b>× {coef} 🎄 Выигрыш {win_amount:.2f} {get_balance_emoji()} ❞</b></blockquote>\n\n"
                f"<blockquote>{emoji} Шанс выигрыша {win_chance:.1f}%, коэффициент: x{coef}\n"
                f"🎟 Ваше число {win_number}, нужно: {coef}</blockquote>\n\n"
                f"<b>📋 Баланс {new_balance:.2f} {get_balance_emoji()}</b>"
            )
        else:
            # Начисляем рефские 5% (60/30/10)
            referrer_id = user_data[12]
            if referrer_id:
                # Начисляем 5% от проигрыша (как в оригинале)
                db.add_ref_balance(referrer_id, bet * 0.05)
                
            win_chance = 100 / coef
            text = (
                f"<b>👤 {user_name}</b>\n"
                f"<b>Проигрывает в игре {emoji} на {bet:.2f} {get_balance_emoji()}</b>\n"
                f"<blockquote><b>× 0 🎄 Выигрыш 0.00 {get_balance_emoji()} ❞</b></blockquote>\n\n"
                f"<blockquote>{emoji} Шанс выигрыша {win_chance:.1f}%, коэффициент: x{coef}\n"
                f"🎟 Ваше число {win_number}, нужно: {coef}</blockquote>\n\n"
                f"<b>📋 Баланс {new_balance:.2f} {get_balance_emoji()}</b>"
...
async def process_bet_amount(message: Message, state: FSMContext):
    ...
    try:
        amount = float(message.text.replace(",", "."))
        if amount < 0.01:
             return await message.answer(f"❌ Минимальная ставка — <b>0.01 {get_balance_emoji()}</b>")
        if amount > config.MAX_BET:
             return await message.answer(f"❌ Максимальная ставка — <b>{config.MAX_BET:.2f} {get_balance_emoji()}</b>")
    except ValueError:
        return await message.answer("❌ Введите корректную сумму (число больше 0)")

    db.set_bet(user_id, amount)
    await state.set_state(None)
    await message.answer(f"✅ Ставка успешно изменена на <b>{amount:.2f} {get_balance_emoji()}</b>")
...
async def game_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if game_type in ["soccer", "basket", "darts", "bowling", "slots"]:
    ...
    try:
        ...
        if bet_type == "not_6":
            ...
            if balance < potential_loss:
                 await state.update_data(processing_click=False)
                 text = f"❌ Недостаточно средств! При выпадении 6 вы потеряете {potential_loss:.2f} {get_balance_emoji()}.\nНужно иметь эту сумму на балансе."
        ...
        if win_coef > 0:
            ...
            text = (
                f"<b>👤 {user_name}</b>\n"
                f"<b>Побеждает в игре 🎲 на {bet:.2f} {get_balance_emoji()}</b>\n"
                f"<blockquote><b>× {win_coef} 🎄 Выигрыш {win_amount:.2f} {get_balance_emoji()} ❞</b></blockquote>\n\n"
                f"<b>📋 Баланс {new_balance:.2f} {get_balance_emoji()}</b>"
            )
            await state.update_data(processing_click=False)
...
async def main() -> None:
    ...
    print("✅ Поддержка премиум-эмодзи активирована")
    print("✅ Команда /admin доступна для администраторов")
    print("✅ Реферальная система работает (60%/30%/10%)")
    print("✅ Игра '🎲' убрана из меню")
    try:
